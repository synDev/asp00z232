...
public final class Utils {
	...
	private static boolean deleteDir(File dir) {
		...
		public Position[] getInteractablePositions(int size) {
		final Position[] interactivePositions = new Position[size == 1 ? 4 : ((size + 1) * 4)];
		int pointer = 0;
		for (int x = -1; x <= size; x++) {
			for (int y = -1; y <= size; y++) {
				//TODO should this also be done on other sizes?
				if (size == 1) {
					//if {@code size} is 1, exclude all diagonal positions
					if (x == -1 && y == -1 || x == size && y == -1 || y == size && x == -1 || x == size && y == size)
						continue;
				}
				
				if (x >= 0 && x < size && y >= 0 && y < size) {
					//if new position is too close depending on
					//{@code size} - continue to next position
					continue;
				}

				//copy() returns a new Position with the same
				//x, y and z coords from this position.
				Position pos = copy().add(x, y);
				interactivePositions[pointer] = pos;
				pointer++;
			}
		}
