...
public class CombatAssistant {
    ...
    public void applyRecoilNPC(int damage, int i) {
        if (damage <= 0)
            return;
        if (damage > 0) {
            if (c.npcIndex > 0 && Server.npcHandler.npcs[i] != null) {
                ...
                Server.npcHandler.npcs[i].HP -= recDamage;
                Server.npcHandler.npcs[i].hitDiff2 = recDamage;
    ...
    public int[][] slayerReqs = {{1648, 5}, {2530, 1}, {1832, 7}, {1600, 10}, {1612, 15}, {1831, 17}, {1623, 20}, {1620, 25}, {1633, 30}, {1827, 35}, {1616, 40}, {1643, 45}, {1618, 50}, {1627, 55}, {1624, 65}, {1610, 75}, {1613, 80}, {1615, 85}, {2783, 90}, {3590, 92}, {3591, 82}, {3592, 72}};
    public int[][] slayerItemReqs = {{2530, 1725}, {1616, 4156}, {1620, 4156}, {1612, 4166}};

    public boolean goodSlayer(int i) {
        if (Server.npcHandler.npcs[i].npcType == 1627 || Server.npcHandler.npcs[i].npcType == 1608) {
            if (c.playerEquipment[c.playerWeapon] == 4158 || c.playerEquipment[c.playerWeapon] == 3282) {
                return true;
            } else {
                c.sendMessage("You need to be wearing a leaf bladed item to harm this monster.");
                return false;
            }
        }
        for (int j = 0; j < slayerReqs.length; j++) {
            for (int k = 0; k < slayerItemReqs.length; k++) {
                if (slayerReqs[j][0] == Server.npcHandler.npcs[i].npcType) {
                    if (slayerReqs[j][1] > c.playerLevel[c.playerSlayer]) {
                        ...
                        return false;
                    } else if (slayerItemReqs[k][0] == Server.npcHandler.npcs[i].npcType) {
                        if (!c.getItems().isWearing(slayerItemReqs[k][1])) {
                            c.sendMessage("You need to be wearing " + c.getItems().getItemName(slayerItemReqs[k][1]).toLowerCase() + " to harm this monster.");
                            return false;
                        }
                    }
                }
    ...
    public void attackNpc(int i) {
        if (c.playerEquipment[c.playerWeapon] == 4747) {
            c.hitDelay = getHitDelay(c.getItems().getItemName(c.playerEquipment[c.playerWeapon]).toLowerCase());
            c.doubleHit = true;
        }
        if (Server.npcHandler.checkSlayerMask(c, i)) {
        ...
        if (NPCHandler.npcs[i] != null) {
            ...
            boolean sameSpot = c.absX == NPCHandler.npcs[i].getX() && c.absY == NPCHandler.npcs[i].getY();
          ...
          //      if (NPCHandler.pathBlocked(o, c)) {
           ...
           // }
            ...
            if ((c.underAttackBy > 0 || c.underAttackBy2 > 0) && c.underAttackBy2 != i && !c.inMulti()) {
                if (c.underAttackBy2 == 1614 || c.underAttackBy == 1614) {
                    return;
                }
                resetPlayerAttack();
            ...
            if (c.attackTimer <= 0) {
                ...
                boolean usingCross = (c.playerEquipment[c.playerWeapon] == 837 || c.playerEquipment[c.playerWeapon] == 767);
                c.bonusAttack = 0;
                c.rangeItemUsed = 0;
                c.projectileStage = 0;
                if (c.autocasting) {
                ...
                if (correctBowAndArrows() < c.playerEquipment[c.playerArrows] && Config.CORRECT_ARROWS && usingBow && !usingCrystalBow() && c.playerEquipment[c.playerWeapon] != 837 && c.playerEquipment[c.playerWeapon] != 767) {
                ...
                if (c.playerEquipment[c.playerWeapon] == 837 && !properBolts()) {
                ...
                if (c.playerEquipment[c.playerWeapon] == 767 && !properBolts()) {
                    c.sendMessage("You must use bolts with a crossbow.");
                    c.stopMovement();
                    c.freezeTimer = 1;
                    resetPlayerAttack();
                ...
                if (usingBow || c.usingMagic || usingOtherRangeWeapons || (c.goodDistance(c.getX(), c.getY(), NPCHandler.npcs[i].getX(), NPCHandler.npcs[i].getY(), 2) && usingHally()) || (c.goodDistance(c.getX(), c.getY(), NPCHandler.npcs[i].getX(), NPCHandler.npcs[i].getY(), NPCHandler.npcSizes(i)))) {
                    c.stopMovement();
                }
    ...
    public void delayedHit(int i) { // npc hit delay
        ...
        if (NPCHandler.npcs[i] != null) {
            ...
            if (!c.castingMagic && c.projectileStage > 0) { // range hit damage
                ...
                if (damage > 0 && c.fireSpell() && NPCHandler.weakFire(NPCHandler.npcs[i].npcType)) {
                    damage *= 2.00;
                }
        ...
        if (PlayerHandler.players[i] != null) {
            ...
            if (PlayerHandler.players[i] != null) {
                ...
                if (c.attackTimer <= 0) {
                    ...
                    if (correctBowAndArrows() < c.playerEquipment[c.playerArrows] && Config.CORRECT_ARROWS && usingBow && !usingCrystalBow() && c.playerEquipment[c.playerWeapon] != 767 && c.playerEquipment[c.playerWeapon] != 837 && !c.usingMagic) {
                        c.sendMessage("You can't use " + c.getItems().getItemName(c.playerEquipment[c.playerArrows]).toLowerCase() + "s with a " + c.getItems().getItemName(c.playerEquipment[c.playerWeapon]).toLowerCase() + ".");
                        c.stopMovement();
                        resetPlayerAttack();
                        return;
                    }
                    if (c.playerEquipment[c.playerWeapon] == 767 && !properBolts() && !c.usingMagic) {
                        c.sendMessage("You must use bolts with a crossbow.");
                        c.stopMovement();
                        resetPlayerAttack();
                        return;
                    }
                    if (c.playerEquipment[c.playerWeapon] == 837 && !properBolts() && !c.usingMagic) {
                        c.sendMessage("You must use bolts with a crossbow.");
                        c.stopMovement();
                        resetPlayerAttack();
                        return;
    ...
    public boolean checkReqs() {
        ...
        if (!PlayerHandler.players[c.playerIndex].inWild() && c.duelStatus != 5 && !c.usingMagic) {
    ...
    public int getWepAnim(String weaponName) {
        ...
        if (weaponName.endsWith("scimitar") || weaponName.contains("machete")) {
            switch (c.fightMode) {
                case 0:
                case 2:
                case 1:
        ...
        if (weaponName.endsWith("longsword")) {
            switch (c.fightMode) {
                case 0:
                case 1:
                case 2:
        ...
        if (weaponName.endsWith("sword") || weaponName.contains("-ak")) {
            switch (c.fightMode) {
                case 0:
                case 3:
                case 1:
        ...
        if (weaponName.contains("bow") && !weaponName.contains("'bow") && !weaponName.contains("cross")) {
            return 426;
        }
        if (weaponName.contains("'bow"))
            return 4230;
    ...
    public int getBlockEmote(String equip) {
        if (equip.contains("shield") || equip.contains("ket-xil")) {
            return 1156;
        }
    ...
    public int calculateMeleeAttack() {
        ...
        i += c.bonusAttack;
        if (c.slayerHelmAffect && c.slayerTask != 0) {
            i *= 1.15;
        }
        if (c.fullVoidMelee())
            attackLevel += c.getLevelForXP(c.playerXP[c.playerAttack]) * 0.1;
    ...
    public boolean usingBolts() {
        return c.playerEquipment[c.playerArrows] == 877 || c.playerEquipment[c.playerArrows] == 879 || c.playerEquipment[c.playerArrows] == 880 || c.playerEquipment[c.playerArrows] == 881;
    }
    ...
    public int rangeMaxHit() {
        ...
        if (c.fullVoidRange())
            modifier += 0.20;
        if (c.playerEquipment[c.playerCape] == 5607)
            modifier += 1000.50;
    ...
    public int getRangeProjectileGFX() {
        ...
        if (c.playerEquipment[c.playerWeapon] == 837 || c.playerEquipment[c.playerWeapon] == 767)
            return 27;
        switch (c.rangeItemUsed) {
    ...
    public int mageAtk() {
        ...
        if (c.prayerActive[4])
            ...
            attackLevel *= 1.15;
        if (c.MAGIC_SPELLS[c.oldSpellId][0] == 12037)
            attackLevel *= 3.37;
        for (int i = 0; i < NPCHandler.maxNPCs; i++) {
            if (NPCHandler.npcs[i] != null) {

                if (c.fireSpell() && NPCHandler.weakFire(NPCHandler.npcs[i].npcType))
                    attackLevel *= 3.01;
                if (NPCHandler.weakAFBarrows(NPCHandler.npcs[i].npcType))
                    attackLevel *= 50.01;
            }
        }
